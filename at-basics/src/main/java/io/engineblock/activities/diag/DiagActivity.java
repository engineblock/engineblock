...
public class DiagActivity extends SimpleActivity implements Activity, ActivityDefObserver {
    ...
    public synchronized void onActivityDefUpdate(ActivityDef activityDef) {
        ...
        activityDef.getParams().getOptionalDouble("diagrate").ifPresent(
                rate -> {
                    if (diagRateLimiter == null) {
                        diagRateLimiter = new CoreRateLimiter(rate);
                    } else {
                        if (diagRateLimiter.getRate() != rate) {
                            diagRateLimiter.setRate(rate);
                        }
                    }
                }
        );
